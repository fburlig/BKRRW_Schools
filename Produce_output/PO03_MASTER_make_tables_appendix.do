...
file open myfile using "$dirpath_results_final/Appendix/tab_heterogeneity_eb.tex", write replace
...
** Table C.1: Effects of bond measures on energy use in untreated schools
{
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_regressions_bonds.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.2: Panel fixed effects results (all hourly)
{
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_regressions_binary_davis_reguant_allhourly.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.3: Panel FE (Alternative SEs)
{
...
** Table C.4: Panel FE results (average school specific estimates; outliers)
{
use "$dirpath_data_int/RESULTS_monthly_wtemperature.dta", clear
replace spec = 7 if spec == 6
keep if spec==7
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_regressions_binary_reguant_samples.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.5: Panel fixed effects results (donuts -- davis)
{
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_regressions_binary_davis_donuts.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.6: Panel fixed effects results (donuts -- reguant)
{
use "$dirpath_data_int/RESULTS_monthly_DONUTS.dta", clear
keep if xvar =="savings binary" & yvar == "qkw_hour" & subsample== "0"
keep if postctrls == ""
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_regressions_binary_reguant_donuts.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.7: Panel fixed effects results (continuous treatment variable)
{
use "$dirpath_data_int/RESULTS_monthly_wtemperature.dta", clear
replace spec = 7 if spec == 6
keep if spec==7
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_regressions_continuous_davis_reguant.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.9: R^2s of prediction models across ML methods
{
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_mlmethods_r2.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.10: Machine learning results (all hourly)
{
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_predictions_binary_davis_reguant_allhourly.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.12: Machine learning results (bootstrap)
{
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_predictions_bootstrap.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.13: Machine learning effects (average school specific estimates; outliers)
{
use "$dirpath_data_int/RESULTS_monthly.dta", clear
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_predictions_binary_reguant_samples.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.14: Machine learning effects results (donuts -- davis)
{
use "$dirpath_data_int/RESULTS_monthly_DONUTS.dta", clear
keep if xvar =="davis binary" & yvar == "prediction_error4" & subsample== "0"
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_predictions_binary_davis_donuts.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.15: Machine learning effects results (donuts -- reguant)
{
use "$dirpath_data_int/RESULTS_monthly_DONUTS.dta", clear
keep if xvar =="savings binary" & yvar == "prediction_error4" & subsample== "0"
keep if postctrls == "post"
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_predictions_binary_reguant_donuts.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.16: Machine learning results (continuous treatment variable)
{
use "$dirpath_data_int/RESULTS_monthly.dta", clear
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_predictions_continuous_davis_reguant.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
...
** Table C.17: Machine learning effects (alternative prediction methods)
{
...
** Table C.18: Machine learning results (double machine learning)
{
...
capture file close myfile
file open myfile using "$dirpath_results_final/Appendix/tab_aggregate_predictions_dl.tex", write replace
if "`standalone'" == "_standalone" {
 file write myfile "\documentclass[12pt]{article}" _n
 file write myfile "\usepackage{amsmath}" _n
 file write myfile "\usepackage{tabularx}" _n
